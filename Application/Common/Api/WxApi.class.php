...
class WxApi {
    ...
    private function getCode($redirect_uri = '', $scope = 'snsapi_base', $state = '1') {
          $redirect_uri = empty($redirect_uri) ? 'http://' . trim($_SERVER['HTTP_HOST'],':8080') . $_SERVER['PHP_SELF'] . '?' . $_SERVER['QUERY_STRING'] : $redirect_uri;
    ...
    public function getSubscribeUserInfo($openid, $lang = 'zh_CN') {
        ...
        $result = json_decode($result_data, true);
        if (APP_DEBUG) {
            file_put_contents('data_info.log', "访问时间：" . date('Y-m-d H:i:s') . "\n" . "用户信息：" . $result_data . "\n", FILE_APPEND);
        }
